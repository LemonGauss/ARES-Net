    ...
    def __init__(self,
        ...
        for i_layer in range(self.num_layers):
            ...
            layer = MMBasicLayer(
                ...
                attention_window=attention_windows[i_layer],
                # # ==============================================================
            )
...
class MMBasicLayer(nn.Module):
    def __init__(self,
        ...
        # self.fusion = VATF(vis_dim=dim,
        
        
        #            num_heads=num_heads_fusion,
        #            dropout=fusion_drop)
        # self.fusion = ImprovedCrossAttention(dim,768,dim,num_heads_fusion,fusion_drop)
        
        self.ARattention_window=attention_window
        self.fusion = AR_Longformer_PWAM(dim,          
                            dim,          # v_in_channels
                            768,          
                            dim,          # key_channels
                            dim,          # value_channels
                            num_heads=num_heads_fusion,
                            dropout=fusion_drop,
                            attention_window=self.ARattention_window)
...
class SpatialImageLanguageAttention(nn.Module):
    ...
    def forward(self, x, l, l_mask):
        ...
        self.last_attn_map = sim_map.detach().cpu()
        ...
        self.last_out_feat = out.detach().cpu()
...
class LongformerAudioRefinementBlock(nn.Module):

    def __init__(self, d_model, nhead, dropout=0.1, attention_window=32):
        ...
        self.attlist=[attention_window]
        config = LongformerConfig(
            hidden_size=d_model,
            num_attention_heads=nhead,
            attention_window=self.attlist,  
            num_hidden_layers=1,                
            attention_probs_dropout_prob=dropout,
        )
        # ========================================================

        self.self_attn = LongformerSelfAttention(config, layer_id=0)
    ...
    def forward(self, src, src_key_padding_mask):
            ...
            is_index_masked = torch.full((batch_size, seq_len), False, device=src.device)
            # ====================================================
            ...
            refined_src = self.norm(src + self.dropout(attn_output))

            return refined_src


# ====================================================================================
class AR_Longformer_PWAM(nn.Module):

    def __init__(self, dim, v_in_channels, l_in_channels, key_channels, value_channels, num_heads=1, audio_nhead=8, dropout=0.0, attention_window=32):
        super(AR_Longformer_PWAM, self).__init__()

        
        self.audio_refinement = LongformerAudioRefinementBlock(
            d_model=l_in_channels, 
            nhead=audio_nhead, 
            dropout=dropout,
        ...
        self.image_lang_att = SpatialImageLanguageAttention(
            v_in_channels=v_in_channels, l_in_channels=l_in_channels,
            key_channels=key_channels, value_channels=value_channels,
            out_channels=value_channels, num_heads=num_heads
        )
        
        
        self.project_mm = nn.Sequential(
            nn.Conv1d(value_channels, value_channels, kernel_size=1),
            nn.GELU(),
            nn.Dropout(dropout)
        )
        # [NEW] Container to expose visualization data to the outside script
        self.last_out_feat = None
        self.last_attn_map = None

    def forward(self, x, l, l_mask):
            audio_feat = l.permute(0, 2, 1)
            audio_pad_mask = (l_mask.squeeze(-1) == 0)

            refined_audio_feat = self.audio_refinement(audio_feat, audio_pad_mask)
            refined_l = refined_audio_feat.permute(0, 2, 1)

            vis = self.vis_project(x.permute(0, 2, 1))
            
            # Call the attention module
            # Note: SpatialImageLanguageAttention's forward returns (B, HW, C)
            lang_out = self.image_lang_att(x, refined_l, l_mask)
            
            # ==================== CRITICAL FIX ====================
            # Copy the visualization data from the sub-module to this module
            # so the external script can access model.layers[-1].fusion.last_out_feat
            if hasattr(self.image_lang_att, 'last_out_feat'):
                self.last_out_feat = self.image_lang_att.last_out_feat
            
            if hasattr(self.image_lang_att, 'last_attn_map'):
                self.last_attn_map = self.image_lang_att.last_attn_map
            # ======================================================

            lang = lang_out.permute(0, 2, 1) # (B, C, HW)
            
            mm = torch.mul(vis, lang)
            mm = self.project_mm(mm).permute(0, 2, 1)
            return mm


class AudioRefinementBlock(nn.Module):
...
class AR_PWAMraw(nn.Module):

    def __init__(self, dim, v_in_channels, l_in_channels, key_channels, value_channels, num_heads=1, dropout=0.0):
        super(AR_PWAMraw, self).__init__()
        
        
        
        self.audio_refinement = AudioRefinementBlock(d_model=l_in_channels, nhead=8, dropout=dropout)
    ...
    def forward(self, x, l, l_mask):
        ...
        audio_pad_mask = (l_mask.squeeze(-1) == 0) # (B, N_l)
        ...
        lang = self.image_lang_att(x, refined_l, l_mask) # (B, H*W, dim)
        lang = lang.permute(0, 2, 1) # (B, dim, H*W)
